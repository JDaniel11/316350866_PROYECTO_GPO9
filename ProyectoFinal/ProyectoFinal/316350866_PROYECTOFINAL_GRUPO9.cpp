...
// Conjunto de de variables que nos permiten controlar la posicion del spray y la rotacion del mismo y de la tapa.
...
typedef struct _frame
{
	...
	float rotSpray; //Variable para rotacion de spray
	float rotIncSpray; //Incremento de la rotacion del spray
	float rotTapa; //Variable para la rotacion de la tapa del botiquin
	float rotInc; //Incremento para la rotacion de la tapa de botiquin
...
void saveFrame(void) //Guardado de frames y con el, el valor de las variables que permiten la animacion por keyframe.
{
...
void resetElements(void) //Reseteo de las posiciones al keyframe 0 para terminar la animacion
{
...
void interpolation(void) //Interpolacion de valores para hacer el movimiento de las animaciones por keyframe.
{
...
//Funcion hecha para el traspaso de valores de posicion y rotacion del spray y de la tapa botiquin.
...
int main()
{
	...
	//Cargado de texturas para el skybox
	...
	setFramesValues(-5.3, 1, -1.5,0,0); //Posiciones iniciales
	saveFrame();
	
	setFramesValues(-5.3, 1, -1.5, 30,0); //Apertura de la tapa del botiquin
	saveFrame();

	setFramesValues(-5.3, 1.5, -1.5, 30,0); //Alza del spray de primeros auxilios
	saveFrame();

	setFramesValues(-5.0, 1.5, -0.5, 30,0); //Movimiento que saca el spray del botiquin
	saveFrame();

	setFramesValues(-5.0, 2.5, -0.5, 30, 0); //Alza del spray a la altura de la mesa
	saveFrame();

	setFramesValues(-3.5, 2.5, -0.5, 30, 90); //Rotacion y puesta del spray encima de la mesa
	saveFrame();

	setFramesValues(-3.5, 2.5, -0.5, 30, 90); //Se mantiene en la posicion pasada
	saveFrame();

	setFramesValues(-5.3, 1, -1.5, 0,0); //Vuelta a la posición inicial
	saveFrame();
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_O]) //MOVIMIENTO LETRERO
	{
		if (bandaMov == false)
		{
			bandaMov = true;
	...
	if (keys[GLFW_KEY_K]) //MOVIMIENTO BURBUJA
	{
		if (burbujaMov == false)
		{
	...
	if (keys[GLFW_KEY_P]) //MOVIMIENTO POKEBOLAS
	{
		if (seMueve == false)
		{
			seMueve = true;
...
void DoMovement()
{
	...
	if (bandaMov == true) //MOVIMIENTO DEL LETRO CIRCULAR
	{
		rot += 1.0f;
	}

	if (burbujaMov == true) //MOVIMIENTO DE LAS BURBUJAS
	{
		if (burbujaVert < 6.0f)
		{
			burbujaVert += 0.01f;
	...
	if (seMueve)  //MOVIMIENTO DE LA POKEBOLA
	{
